...
class HumidReadingsAdapter(private var readingsList: List<Readings>) : RecyclerView.Adapter<HumidReadingsAdapter.ReadingViewHolder>() {
    ...
    inner class ReadingViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(readings: Readings, position: Int) {
            with(itemView) {
                ...
                internalTempView.text = readings.internalHumidity.toString()
                externalTempView.text = readings.externalHumidity.toString()
